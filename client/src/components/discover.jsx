...
import BricksImage from './../assets/image 14.png';
import { to2dp } from '../utils/to2dp';


export const BED_ICON = (
...
const Discover = () => {
  ...
  const fetchAssets = () => {
    ...
    bricks.fetchAllAssets().then((assets)=>{
      setAssets(assets.filter((asset)=>asset.key != "JB491zZDF4b58kzGnNX32gpdZaQ9HECUNZqtXdhKCngh"));
    });
  }
    ...
    <div className="bg-white min-h-screen">
      ...
      <section className="bg-gradient-to-r from-[#C9E1E7] to-[#FCDBBB] px-8 py-12 rounded-lg mx-6 my-8">
        <div className="flex relative">
          ...
          <div className='w-1/3 absolute right-0 top-[-40px]'>
            <img
              src={BricksImage} // Replace with actual image URL
      ...
      <section className="px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-6">
          {assets.map((asset) => (
            <Link to={`/discover/asset/${asset.key}`} key={asset.id} className="bg-white p-4 rounded-t-[12px] cursor-pointer">
              ...
              <div className="mt-4 flex flex-col gap-2">
                <h3 className="text-[24px] font-bold text-[#E4AA15]">${to2dp(asset.value*(rate || 0))}</h3>
                ...
                <div className="flex items-center gap-3 mt-2 font-jakarta font-medium text-[14px]">
                  {
                    asset.attributes.map(({key, value}) => {
                      ...
                      return <div className="flex items-center gap-1 text-[#000929]">
                        {icon}
                        {value}
                      </div>
                    })
