...
export default function DiscoverAsset(){
    ...
    const initFinancials = (asset)=>{
        setFinancials([
            {
                title: "Property Price (SOL)",
                value: `${asset.value} SOL`
            },
            {
                title: "Property Price ($)",
                value: `$${to2dp(asset.value * (rate || 0))}`
            },
            {
                title: "Purchase Value (SOL)",
                isInput: true
            }
        ])
    }
    ...
    const { post: saveAccountKey } = useAPI();
    ...
    const createAccount = async ()=>{
        if(publicKey){
            ...
            const createPromise = new Promise(async (resolve, reject) => {
                ...
                bricks.initializeUser(accountId).then(async ({hash, key})=>{
                    const postData = {
                        account_key: key
                    }
                    await saveAccountKey("api/user/update/key", postData, true, false)
                    setAccountKey(key);
                    resolve();
            ...
            toast.promise(createPromise, {
                    ...
                    error: "An error occured while creating your account onchain"
            }, {
                position: "bottom-right"
            })
        }
    ...
    const fetchUser = async ()=>{
        if(accountKey){
            ...
            bricks.fetchUser({userKey: accountKey}).then((user)=>{
                const index = user.owned_assets.indexOf(params.id);
                if(index !== -1){
                    setAmountOwned(user.ownership_amounts[index]);
                }
            });
        }
    }
    ...
    const buyAsset = async ()=>{
        if(publicKey && accountKey){
            setBuyingAsset(true);
            const buyingPromise = new Promise((resolve, reject) => {
                const bricks = new BricksProgram(connection, publicKey, signTransaction);
                bricks.buyAsset({
                    user_account: accountKey,
                    amount: buyValue,
                    asset_key: params.id
                }).then((tx)=>{
                    resolve();
                    toast(`Transaction Hash ${tx}`, {
                        position: "bottom-right",
                        autoClose: 10000,
                        hideProgressBar: true,
                        closeOnClick: false,
            ...
            toast.promise(buyingPromise, {
                    pending: "Purchasing Asset",
                    error: "An error occured while purchasing the asset"
            },{
                position: "bottom-right"
            })
        }
        else if(publicKey && !accountKey) {
            createAccount();
        }
    }
            ...
            <div className="flex w-full gap-8">
                <div className="w-[60%] flex flex-col gap-6">
                    ...
                    <div className="flex items-center gap-8">
                        ...
                        <div className="w-full max-w-[200px] flex p-3 items-center gap-3 bg-[#DDA72233] rounded-lg">
                            <svg width="26" height="26" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className=" border-2 border-[#D9D9D9] rounded-2xl p-3  gap-2 flex flex-col w-full max-w-[40%] font-jakarta">
                            <div className="flex justify-between items-center w-full">
                                <div className="text-[#807F7F] text-[14px] font-inter font-medium">Amount Owned ($)</div>
                                <div className="flex items-center gap-2 text-[15px] font-inter font-semibold">${to2dp(amountOwned*(rate || 0))}</div>
                            </div>
                            <div className="flex justify-between items-center w-full">
                                <div className="text-[#807F7F] text-[14px] font-inter font-medium">Pecentage Owned</div>
                                <div className="flex items-center gap-2 text-[15px] font-inter font-semibold">{to2dp(amountOwned / asset.value * 100)}%</div>
                            </div>
                    </div>

                    <div className="font-jakarta flex items-center gap-4">
                        <span className="text-[23px] text-[#E4AA15] font-bold">${to2dp(asset.value * (rate || 0))}</span>
                    ...
                    <div className="flex flex-col gap-3 w-full">
                        <div className="relative w-full">
                            ...
                            <div className="absolute top-0 h-[4px] w-full rounded-lg z-[1]" style={{background: `linear-gradient(90deg, #E4AA15, #E4AA15) 0% 0% / ${100 - ((asset.value - asset.value_bought)/asset.value * 100)}% 100% no-repeat`}}></div>
                        </div>
                        <div className="flex items-center font-jakarta justify-between">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="text-[#E4AA15] font-medium text-[15px]">{to2dp(100 - ((asset.value - asset.value_bought)/asset.value * 100))}% funded</div>
                            </div>
                ...
                <div className="flex-1">
                    <div className="mt-[100px] flex flex-col items-center gap-5 px-6 py-8 rounded-[32px]" style={{boxShadow:'1px 1px 32px -8px #0000001A'}}>
                        ...
                        <div className="flex flex-col gap-2 items-center">
                            ...
                            <div className="font-jakarta font-bold text-[22px]">${to2dp(asset.value * (rate || 0))}</div>
                        </div>
                        <div className="relative bg-[#F5F5F5] rounded-lg flex flex-col gap-4 py-6 px-5 w-full">
                            <div className="flex items-center justify-between">
                                <div className="font-jakarta text-[14px] font-medium">Percentage</div>
                                <div className="font-jakarta font-semibold text-[#E4AA15]">{to2dp(buyValue/asset.value * 100 || 0)}%</div>
                            </div>
                            <div className="flex items-center justify-between">
                                <div className="font-jakarta text-[14px] font-medium">Amount (SOL)</div>
                                <div className="font-jakarta font-semibold text-[#E4AA15]">{buyValue || 0} SOL</div>
                            </div>
                            <div className="flex items-center justify-between">
                                <div className="font-jakarta text-[14px] font-medium">Amount</div>
                                <div className="font-jakarta font-semibold text-[#E4AA15]">${to2dp(buyValue * (rate || 0) || 0)}</div>
                            </div>
                            <div className="p-5"></div>
                            <div className="text-[14px] font-inter text-red-500 font-medium">
                            {
                                    !(asset.value - asset.value_bought >= buyValue && buyValue !== 0 ) ? <>Enter valid purchase value to continue</> : <></>
                            }
                            </div>
                            <div className="w-full absolute bottom-[-30px] flex justify-center">
                                ...
                                <div className="rounded-xl text-[#FFFFFF] py-3 px-9 cursor-pointer">
                                    {!publicKey ?
                                        ...
                                        (accountKey == null ? 
                                            <button className="bottom-0 bg-[#E4AA15] rounded-xl text-[#FFFFFF] py-3 px-9 cursor-pointer min-w-[180px]" disabled={creatingAccount} onClick={createAccount}>Create Account</button>
                                            : 
                                        <button className="bottom-0 bg-[#E4AA15] rounded-xl text-[#FFFFFF] py-3 px-9 cursor-pointer w-[180px]" disabled={buyingAsset} onClick={buyAsset}>Buy</button>
                                        )
                                    }
                                </div>
                                 }
