                        ...
                        <div className="font-bold text-[20px] tracking-[1%]">Financials</div>
                        {
                            financials.map(({title, value, isTest}, key) => {
                                return <div className="flex items-center justify-between" key={key}>
                                    ...
                                    {isTest ? <input type="number" className="border-[1px]" placeholder="Amount" max={asset.value - asset.value_bought} onChange={e=>setBuyValue(e.target.value)} /> : <div className="text-[13px] font-medium">{value}</div>}
                                </div>
                            })
                        }
                    </div>
