  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if(response['success']){
        ...
        const authCookie = new CookieData('authToken', data["token"], createCookieDate(data["expiresIn"]));
          ...
          if(responseData["success"] === true){
            try {
              ...
              const key = PublicKey(accountKey);
              const keyCookie = new CookieData('accountKey', key, createCookieDate(data["expiresIn"]));
              const idCookie = new CookieData('accountId', userId, createCookieDate(data["expiresIn"]));      
                jtCookies.saveCookies( [keyCookie, idCookie] );
            }
            catch {
