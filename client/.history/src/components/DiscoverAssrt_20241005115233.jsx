            ...
            <div className="flex w-full gap-8">
                <div className="w-[60%] flex flex-col gap-6">
                    ...
                    <div className="flex flex-col gap-3 font-jakarta">
                        ...
                        {
                            financials.map(({title, value, isTest}, key) => {
                                return <div className="flex items-center justify-between" key={key}>
                                    <div className="font-medium text-[15px] tracking-[1%]">{title}</div>
                                    {isInput ? <input type="number" className="border-[1px]" placeholder="Amount" max={asset.value - asset.value_bought} onChange={e=>setBuyValue(e.target.value)} /> : <div className="text-[13px] font-medium">{value}</div>}
                                </div>
                            })
                        }
                    </div>
