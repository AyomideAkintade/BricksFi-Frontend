...
export default function DiscoverAsset(){
    ...
    return (
        <div className="flex flex-col gap-3">
            <div className="flex w-full gap-6 h-[60vh]">
                <div className="w-full h-full">
                    <div className="h-full">
                        <img className="rounded-[16px] h-full" src={asset.images[1]} alt={asset.name} />
                    </div>
                </div>
                <div className="w-full grid grid-cols-2 gap-6">
                    {
                        asset.images.slice(1).map((imageLink, key)=>{
                            return <img className="rounded-[16px] h-full" key={key} src={imageLink} alt={asset.name} />
                        })
                    }
                </div>
            </div>
